...
class gompAI:
   ...
   docs_collection = Chroma("docs_collection", embedding_function=openai_embeddings, persist_directory=config.vectordb_path)
...
class GompAgent:
  vectordb_path = config.vectordb_path
  huggingface_embeddings = HuggingFaceEmbeddings(model_name=config.embedding_model)
  docs_collection = Chroma("docs_collection", embedding_function=huggingface_embeddings, persist_directory=config.vectordb_path)
  user_collection = Chroma(config.userID, embedding_function=huggingface_embeddings, persist_directory=config.vectordb_path) # will move this down to instance variable when db is setup
  ...
  def _add_catalog(self):
    ...
    # embed the chunks
    catalog_embeddings = self.huggingface_embeddings.embed_documents(catalog_texts)
    # create unique ids
  ...
  def _add_guide(self):
    ...
    # embed the chunks
    guide_embeddings = self.huggingface_embeddings.embed_documents(guide_texts)
    # create unique ids
